...
const DPI = quantity(100, 'dpi'),
  ...
  WEIGHTS = [0.7, 0.3].map((d) => convert(mul(quantity(d, 'mm'), DPI), NONE)),
...
const init = () => {
  ...
  const ctx = cnvs!.getContext('2d'),
    ...
    largeGlyphs = [] as polyline[],
    smallGlyphs = [] as polyline[];
  ...
  letters.forEach((l, i) => {
    ...
    getGlyphVector(l[0], BASE, [bx, by]).map((line: Line) => {
      largeGlyphs.push(polyline(line));
    });
    ...
    for (let j = 0; j < l.length; j++) {
      ...
      getGlyphVector(l[j], EXTS, [ex, ey]).map((line: Line) => {
        smallGlyphs.push(polyline(line));
      });
    }
  });
  ...
  [...'Plot-writer / An SVG text writer for plotter (MIT License) Nicolas LEBRUN'].forEach(
    (l, i) => {
      getGlyphVector(
        l,
        [EXTS[0] * 0.84, EXTS[1]],
        [MRGN + i * EXTS[0] * 0.812, SIZE[1] - MRGN * 1.5]
      ).map((line: Line) => {
        smallGlyphs.push(polyline(line));
      });
    }
  );
  comp = group({ stroke: "#FEFEFE"}, [
    rect(SIZE, { fill: '#333' }),
    ...
    group({ weight: WEIGHTS[0] }, largeGlyphs),
    group({ weight: WEIGHTS[1] }, smallGlyphs),
  ]);
  ctx!.scale(dpr, dpr);
  draw(ctx, comp);
};
